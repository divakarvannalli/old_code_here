...
bool k=query.prepare("SELECT question  FROM questions WHERE rowid =:x");
query.bindValue(":x",lineNumber);
k=query.exec();
QString question;
int idName = query.record().indexOf("question");
query.next();
question = query.value(idName).toString();

 qDebug()<<"--------->"<<k+"   "<<query.lastError().text() ;
#if 0
         char line[256]={0};
         int count=0;
...
//                qDebug()<<"line no "<<lineNumber;
   ...
   ch=  MainWindow::radio_button(arr[1],ch+1);
...
#endif
         QString path=QString::number(question_paper_clicked);
         path=options_file_path_thread+path;
         ...
         qDebug() << "Error: connection with database fail:question table";
           ...
           QSqlQuery query1(*option_table);
//           QSqlQuery query2(*option_table);
//           QSqlQuery query3(*option_table);
//           QSqlQuery query4(*option_table);
//           QSqlQuery query5(*option_table);

           bool k;//=query1.prepare("SELECT *  FROM option_table ");
          // qDebug()<<"--------->"<<k+"   "<<query1.lastError().text() ;

         //  query1.bindValue(":x",lineNumber);
 k=query1.exec("SELECT *  FROM option_table ");
#define no_of_option 5
 int idName_[no_of_option],idName;
    QString question;
      QByteArray ba;
        for(int c=1;c<=no_of_option;c++)
        {
           ...
           idName_[c-1]= query1.record().indexOf(option);
        ...
        //{
          ...
          for(i=0;i<=lineNumber;i++)
           ...
           query1.next();
            ...
            for(int j=0;j<no_of_option;j++)
            {
          QString question = query1.value(idName_[j]).toString();
           QByteArray ba;
           ba=question.toLatin1();
           strcpy(line,ba.data());
           switch(j)
