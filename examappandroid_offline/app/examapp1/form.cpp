...
int i;
    for( i=0;i<total_Q_paper;i++)
    {
     ...
     question_paper[i].q_object->setFixedSize(50,50);
     y=y+80;

         sprintf(sptr,"QUESTION PAPER %d",i+1);
          question_paper[i].q_object->setText(sptr);

          connect(question_paper[i].q_object, SIGNAL(clicked(bool)), this, SLOT(itwasclicked()));
    }
